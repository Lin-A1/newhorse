...
- 每日活动总结：基于 newhorse work、newhorse、Claude Code 和 Codex 的当日会话，融合为 3–5 行“今日做了什么”简明总结（按项目分组，不列单文件），本地时间 23:00 后自动生成一次；提取已去重并截断，保证质量
...
- FTS5/BM25 检索 + 实体提取与加权 —— **无需嵌入模型**；记忆召回每会话仅注入一次（相关性门控、全局+工作区、缓存安全，不再每轮重复）
- 持久 Reminder、Follow-up、Continuity Grant 与 Companion Plan
- 每日活动总结（覆盖 newhorse、Claude Code、Codex 会话，含归档）现为 3–5 行融合式“今日做了什么”简明总结（按项目分组的结构化 JSON 摘要，不列单文件），显示在会话右侧面板与侧边栏，支持立即生成按钮与 agent 查询工具
...
- 工作台 v2 布局：顶部 90 天贡献热力图（GitHub 绿阶明/暗、居中、星期/月份标签、无数据时按会话数兜底）全宽展示，下方两栏（感知+待办 | 用量+每日总结时间线），窄屏自动折叠为单栏；数据刷新时保持滚动位置，页面不会跳回顶部
