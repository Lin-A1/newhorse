...
- Do not replace private `packages/opencode` imports with `@newhorse/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@newhorse/tui`.
...
Status: Completed. The private `@newhorse/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@newhorse/tui`.
...
- The package has no dependency on `opencode`, `@newhorse/cli`, or
  `@newhorse/core`.
...
primitives now live in `@newhorse/tui`. The legacy host consumes explicit
...
and build inputs through `@newhorse/tui/runtime`. Movable app, component,
...
`@newhorse/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@newhorse/tui`. Configured references resolve through the new
...
`@newhorse/tui`. The legacy host injects a narrow plugin host that retains
...
public `@newhorse/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@newhorse/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@newhorse/core`, or either
  executable package.
...
rg '@newhorse/core|packages/opencode|packages/cli' packages/tui
