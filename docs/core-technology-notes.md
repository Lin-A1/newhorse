...
- **Config**: `NEWHORSE_MEMORY_VECTOR_MODE = auto|brute|off` through the config module; `AppConfig.memoryVector.mode` for hosts.
- **Tests**: brute↔naive-cosine equivalence, scope partition exactness, vec0↔brute equivalence, cross-connection persistence, dims-mismatch AND L2-leftover self-heal, store-level semantic hit with zero keyword overlap, restart rebuild, `off` fallback. (23 memory tests green.)
...
- **Ownership invariant**: a row's `endpoint` is written ONLY by the owning process at register time. A proxy always travels owner-ward; the owner either serves locally or 404s/sweeps its own stale row — **proxy cycles cannot form** (verified shape, not just hope).
- **Liveness**: heartbeat-based. The server refreshes ALL of its rows every 10s (unref'd interval, survives transient lock errors); `sweep` deletes rows whose owner stopped heartbeating (crash / kill -9), so the directory self-heals instead of routing into the void.
...
- **Evidence**: `packages/server/src/cross-process.test.ts` (two REAL servers, one directory file: observe/steer/interrupt from the sibling, 409 split-brain, dead-owner 502+sweep, stale-self sweep, resolver-without-directory unaffected) + `scripts/smoke/cross-process.ts` (8/8, no API key — stub LLM).
