...
- **Whole-argv interpreter scan** (`interpreterSpecial`): any token that is a shell family (`SHELL_NAMES` bounded set + busybox/fish, version-normalized so `bash5.2`/`bash-5.2`/`zsh-5.8`/`dash-0.5`/`ksh-2020` reduce to `bash`/`zsh`/`dash`/`ksh`) or interpreter, when it is the exec head **or** is followed by a code flag / script-path / stdin-redirect / next interpreter → prompt. The tail scan is **unbounded** because transparent `-s`/`-l`/`--` shell flags are repeatable and operand-free (`bash -s -s -l -c id` would otherwise push `-c` past a fixed window). This closes the "un-enumerated wrapper" hole (`taskset bash -c`, `runuser -u root -- bash -c`, `chroot / bin/bash -c`) without an open-ended `*sh` ends-with match.
