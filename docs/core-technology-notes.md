...
- **`followup_task` tool**: `ToolCtx.queryTask(taskId)` folds a child's log — `Session.Settled` → `{state:"settled", finish, text}`; `Session.Created` without settle → `"running"`; neither → `"unknown"`. The butler's `followup_task` surfaces this (task id = the `childSessionId` from spawn_agent). The parent can poll a child's result without touching live memory — durable, restart-safe.
- **Known remaining gaps**: `hub.send`/`interrupt` are still stubs (cross-session effect delivery is M4 SessionManager); parent promotion is a steer (it wakes the parent on its NEXT drain — the parent is not actively re-driven mid-idle); a `wait_agent` tool (block until settled with a clamped timeout) is not yet registered.
- **Real-API verification (2026-08-30)**: `scripts/smoke/real-api.ts` was run against a MiniMax Anthropic-compatible endpoint (`--baseUrl https://api.minimaxi.com/anthropic --model MiniMax-M2`) — all 8 assertions passed (basic turn, multi-turn tool, restart recovery, interrupt, bad-key 401, overflow, builtin tool use). The key is passed ONLY via `ANTHROPIC_API_KEY` env (never written to a file). Run: `ANTHROPIC_API_KEY=... bun run scripts/smoke/real-api.ts --baseUrl https://api.minimaxi.com/anthropic --model MiniMax-M2`.

## 21. Agent roles — identity, capability, specialism (Phase 4)

A subagent can be a NAMED agent: identity (role/specialism), a RESTRICTIVE tool whitelist, a body of specialist instructions, and its own model — borrowed from Codex's `agent_type` role overlay (a child never widens the parent's authority) + Claude Code's `agents/*.md` frontmatter.

- **Definition** (`packages/plugin`): `AgentCapability` gains `body` (markdown after the frontmatter — the specialist instructions), `allowedTools` (restrictive whitelist), `role` (cost-down/scheduling key). Discovery reads `agents/<name>.md`; `parseFrontmatter` now matches hyphenated keys (`allowed-tools`).
- **Resolution** (`packages/runtime/src/agent-resolver.ts`, pure): `resolveAgent` = overlay of def over parent context — `allowedTools` narrows (never widens; a named-but-absent tool is dropped); model precedence `explicit > agent.model > costDown > inherited`. Consumed identically by DAG (`DagDeps.agents?`) and butler spawn (`spawn_agent { agent: <name> }`).
- **Durable claim** (`dag-runner`): the effective model is resolved BEFORE `DAG.NodeStarted`, so `foldDAG.models`/`DagOutcome.models` report the model that actually ran (a per-agent def model is not masked by the pre-overlay one). A node's explicit `agent.model` is the explicit slot and wins.
...
## 25. VectorIndex seam — indexed semantic memory (sqlite-vec + brute force)

§24's cosine path re-read and re-deserialized EVERY embedded BLOB from SQLite on every search (O(N) deserialization per query, no index). §25 makes the vector side a real seam (`memory/vector-index.ts`):

- **Seam**: `VectorIndex { upsert(id, vec, scope?) → boolean, remove, search(q, k, scope?) → [{id, score}], size }` — score is cosine SIMILARITY (higher = closer); every method fails soft (false / [], never throws — the vector path must never fail a write or a search, same rule as EmbeddingProvider). `scope` is the isolation partition (the memory's sessionId): a scoped search ranks only same-scope rows, so one busy session's memories cannot crowd another session out of its top-k (the starvation failure of a naive global KNN in a shared-store server).
- **Two shipping implementations**:
  - `brute` — in-memory `Float32Array` map with per-vector norms precomputed at upsert; linear scan. Always available; the resident cache is itself the perf win (no per-query SQLite scan + deserialization).
  - `sqlite-vec` — the vec0 extension (asg017/sqlite-vec, `optionalDependencies` so absence never breaks install) over a `vec0(id text primary key, scope text partition key, embedding float32[dims] distance_metric=cosine)` table IN the same SQLite file. Probed live under Bun 1.3/1.4: `db.loadExtension(getLoadablePath())` works; **bound params must go through `.all(...)`, not `db.query(sql, params)`** (virtual-table MATCH sees NULL with the latter); **the default distance metric is L2 — cosine must be named explicitly**.
- **Canary self-heal**: the table is DERIVED (the `memory.embedding` BLOB stays canonical). At creation a canary insert + KNN proves dimensionality AND metric (an orthogonal query reports distance 1 under cosine but √2 under an L2 table left by an older build, and a wrong-dims table rejects every upsert silently — both would otherwise fail-soft into an invisible dead index); a failed canary drops and recreates, and the store rebuilds from BLOBs at attach.
- **Wiring**: `attachEmbedder(embedder, tag, { vectorMode })` — `auto` (default; vec0 when loadable, else brute), `brute`, `off` (the legacy per-query scan, kill switch). Index build is async (`#vectorReady`) so a search awaits it and degrades to FTS-only until it lands; dims are learned from the first embedded row (a fresh store defers creation to the first embed). `write`/`delete`/`backfill` keep the index in lockstep; a model-tag switch re-embeds under the new tag (never mixes).
- **Config**: `NEWHORSE_MEMORY_VECTOR_MODE = auto|brute|off` through the config module; `AppConfig.memoryVector.mode` for hosts. `opts.vectorIndex` additionally lets a host INJECT its own index (wins over vectorMode) — the seam's wide mouth. Re-attach rules: a mode/vectorIndex change or a TAG CHANGE resets the derived index (a model switch may change dims; rows re-embed under the new tag); the build is single-flight so concurrent first embeds construct it once; backfill carries the row's session scope (deferred embeddings stay scoped-searchable) and agent/user isolation post-filters the FUSED rows in one place (both paths, no mode-dependent leak).
- **Tests**: brute↔naive-cosine equivalence, scope partition exactness, vec0↔brute equivalence, cross-connection persistence, dims-mismatch AND L2-leftover self-heal, store-level semantic hit with zero keyword overlap, restart rebuild, `off` fallback, backfill scope preservation, agent-axis isolation, host-injected index, tag-switch reset. (27 memory tests green.)

## 26. Cross-process SessionManager — SessionDirectory + server proxy (M4)

The in-process hub routes interrupt/send through a memory Map that dies with the process and is invisible to sibling processes. §26 adds the shared, durable half without touching the hub:

- **Seam** (`runtime/session-directory.ts`): `SessionDirectory { register(id, endpoint, pid?), unregister, lookup → {sessionId, endpoint, pid, heartbeatAt}, heartbeat(endpoint), sweep(maxAgeMs) → ids[], entries() }`. Default impl: one shared SQLite file in WAL with `busy_timeout=5000` (multiple processes register/sweep concurrently). `register` is an idempotent upsert — an owner change (re-created session) overwrites.
- **Ownership invariant**: a row's `endpoint` is written ONLY by the owning process at register time. A proxy always travels owner-ward; the owner either serves locally or 404s/sweeps its own stale row — **proxy cycles cannot form** (verified shape, not just hope). `register` RETURNS the previous row so a caller that just took a foreign FRESH row can roll back (the server discards the local App and 409s — the check-then-act race on create is closed by this, not by the pre-check alone); a STALE foreign row (owner past the heartbeat window) is legitimate takeover.
- **Sweep policy (review-hardened)**: a proxy failure is NOT a dead owner. The proxy sweeps only when (a) our own client did not abort the request, AND (b) an independent `GET /v1/health` probe of the owner ALSO fails (2s). A slow-but-alive owner is never dropped; a crashed one is. The holder re-asserts its row on every local hit (idempotent re-register), so a wrongful sweep self-heals in the owner-only-writer direction. The 10s heartbeat tick also runs `sweep(30_000)` (three missed heartbeats + stall margin) — dead rows disappear even when nobody proxies them.
- **Liveness**: heartbeat-based. The server refreshes ALL of its rows every 10s (unref'd interval, survives transient lock errors); `sweep` deletes rows whose owner stopped heartbeating (crash / kill -9), so the directory self-heals instead of routing into the void. `stop()` waits (bounded 2s) for in-flight prompts to settle instead of a blind sleep, and unregister contention can never skip the event-store shutdown.
- **Server routing** (`server.ts`): `findSession` is now a three-way resolve — local map → directory (live elsewhere) → host `sessionResolver` (lazy re-attach) → honest 404. Remote hits PROXY over HTTP: steer/interrupt/snapshot/events are JSON relays (5s timeout); prompt is an SSE stream pass-through (the owner's `req.signal` abort wiring carries client disconnects through the proxy). A stale SELF-entry (endpoint == ours but no App — the owner restarted) is swept and reported missing, never self-proxied. A failed proxy unregisters the entry (502 + self-heal). `POST /v1/session` with an id owned by a sibling returns 409 — two Apps must never drive one log (split-brain guard). New `GET /v1/live` exposes the directory.
- **Lifecycle**: sessions created on a server are registered (owned set) and unregistered on `stop()`; the directory file itself belongs to the caller.
- **Transport-level fixes the work surfaced**: Bun.serve flushes response headers only on the first body byte — an SSE prompt that goes quiet (hung LLM, long tool) hung clients AND proxies until the first event; `promptStream` now emits an immediate `: open` comment + 15s keepalive comments, and `idleTimeout` is configurable (default 120s, Bun default was 10s — long tool turns would have been killed mid-stream). Tests that awaited `fetch(prompt)` as implicit synchronization now consume the stream to `[DONE]` (the correct client behavior).
- **Config**: `NEWHORSE_REGISTRY=<shared sqlite path>` + `NEWHORSE_ADVERTISE_URL` (default derived from host:port). Unset → exactly the old single-process behavior.
- **Honest scope**: cross-process SPAWN-DRIVE is deferred (a child is driven by the process that created it — but the child IS registered, so any sibling can interrupt/steer/observe it); cross-process listing/audit aggregation is not routed through the directory yet.
- **Evidence**: `packages/server/src/cross-process.test.ts` (two REAL servers, one directory file: observe/steer/interrupt from the sibling, 409 split-brain fresh+rollback, dead-owner 502+sweep, slow-owner NOT swept (health gate), stale-self sweep, re-assert on local hit, stale-takeover, resolver-without-directory unaffected) + `scripts/smoke/cross-process.ts` (8/8, no API key — stub LLM).

## 27. Context compaction — local fold + LLM summarizer seam + MODEL-RELATIVE trigger

Long-horizon sessions need the request window bounded without remote-only behavior (codex's remote compaction is explicitly rejected — pillar #1). Compaction is a CORE mechanism (`core/agent/compaction.ts`), driven per turn by the loop:

- **Two layers**: a LOCAL fold (always available, no LLM) keeps the newest `retain` messages (default 12) verbatim and collapses the older head into a single `[previous context: N messages folded...]` marker; the optional LLM summarizer (a seam) instead produces a <200-word objective/decisions/state summary of that head. `AppConfig` wires `compactSummarize` to the session's OWN LLM (provider-agnostic — any LlmClient, the same canonical vocabulary), with a 10s timeout race; ANY failure falls back to the cheap local marker — compaction never blocks or fails a turn.
- **Durable, projection-shaped**: the fold appends a `Session.Compacted { boundarySeq, summary }` event (append-only; "model-visible ⟺ logged" holds — no in-memory history rewrite). Read-time projection (`projectCompacted`) drops only what the summary marker already represents (seq ≤ boundary), keeping the tail verbatim. Restarts reconstruct the exact model view from the log.
- **The trigger is MODEL-RELATIVE (window-aware)**: `compactLimit(opts)` resolves the per-turn char budget as `explicit compactThreshold` > `contextWindowTokens × charsPerToken × 0.6` > `80_000` fallback. Rationale: a fixed 80k chars OVERFLOWS a 32k-token model before the trigger fires and SUMMARY-SPAMS a 200k-token model. `contextWindowTokens` is supplied by the HOST/model config (`AppConfig.contextWindowTokens`, `NEWHORSE_CONTEXT_WINDOW`, per-session `SessionCreateRequest`) — deliberately NOT a built-in per-model table in core (that would make the engine captive to a model registry). Chars stay the measure (no tokenizer dependency); `charsPerToken` (default 2.5, conservative mixed content) converts. The 0.6 fraction leaves headroom for the turn's own reply + tool outputs. Because the threshold derives per run from the CURRENT model, a mid-session switch to a smaller model converges smoothly (the folded head is plain text — no provider-specific format leaks, pillar #4's model-relative lowering).
- **Re-firing**: the trigger measures the PROJECTED visible history, so after a fold the tail grows and re-crosses the (derived) budget — a session cannot outgrow its window after the first compaction. Goal budgets survive compaction by construction (`Session.StepEnded` is never pruned).
- **Scale-aware companions (the same audit round)**: three more fixed constants of the same "model-blind" class were replaced with window-derived/config-driven values:
  - **Output budget**: the anthropic protocol REQUIRES `max_tokens`; unset, every reply silently truncated at a 4096 floor (indistinguishable from the model stopping early), while the openai protocols used the model's own default — protocol-asymmetric behavior for the same session. Now `RunOptions/AppConfig.maxOutputTokens` (`NEWHORSE_MAX_OUTPUT_TOKENS`, per-session DTO) flows into `LLMRequest.maxTokens`; the 4096 floor survives only as the required-field last resort, documented in the protocol.
  - **Summarizer prompt + timeout**: the head text handed to the summarizer was hard-capped at 30k chars and raced against a FIXED 10s timeout — on big heads the LLM summary silently degraded to the local marker exactly where summary quality matters most. Both now scale: `summarizeMaxChars` (window-derived from the loop) and `summarizeTimeoutMs = max(10s, ceil(chars/2500)s)` (pure, exported, tested).
  - **Tail byte budget**: `retain=12` counted MESSAGES, making no promise about tail SIZE — one 50k-char file-read twelve times over still overflowed a small window, AND (a latent deadlock with the window-aware trigger) `compactSession` early-returned "nothing to compact" whenever the count fit, so the trigger fired every turn without ever folding. The kept tail is now capped by BOTH count (`retain`) and bytes (`maxTailChars`, window-derived ×0.3); overflow folds into the summarized head, never silently dropped.
- **Tests**: loop.test.ts — model-relative fire (small window folds where the 80k default would not), fallback no-fire (same history, unknown window), explicit-threshold precedence, `compactLimit` derivation table; compaction.test.ts — fold/projection/summarizer-fallback semantics, byte-budget deadlock fix, summarizeMaxChars cap, `summarizeTimeoutMs` scaling; adapter.test.ts — anthropic explicit-budget passthrough, 4096 floor only when unknown, openai omission symmetry.

## 28. Provider presets (ccswitch layer) + fixed butler role + durable per-session policy

Three small mechanisms landed together on the harness/config floor; all three follow the same rule: **the log / config file is the authority, live state is reconstructable**.

- **Provider presets (ccswitch semantics)** (`runtime/config.ts`): the agent-home config gains `providers: ProviderProfile[]` + `activeProviderId`. A preset is a COMPLETE configuration (kind/baseUrl/apiKey/model/contextWindowTokens/maxOutputTokens) — activating one makes it THE file-layer provider, so model + window budgets switch atomically with the endpoint and can never drift apart (the "上下文预算 vs 模型" coupling). Layering is unchanged: defaults < **preset (file)** < env < cli. Write semantics in `writeAgentHomeConfig`: upsert per field by id (an absent OR empty-string `apiKey` keeps the stored key — the server-side guard, not client cooperation); `providersRemove` drops ids and a dangling `activeProviderId`, and is itself a write-instruction that is never persisted; writes are serialized through a module-level queue (read-modify-write races) and atomic (tmp + rename — a crash mid-write must not truncate the file into "no stored keys"). The client sees only `redactSettings`'s projection (presence + last-4 hint).
- **Fixed butler session role** (`runtime/app.ts` + `core/session/registry.ts` + `runtime/context.ts`): a session created with `asButler` (transport DTO `POST /v1/session {asButler}`) logs `Session.Created {role:"butler"}` (registry fold → `SessionRow.role` → client badge) and composes its system context through `withRoleBody(defaultContextProvider, BUTLER_BODY)` — the coordinator identity leads, the workspace context follows unchanged; children spawned BY the butler never inherit the body (they get their own composition). The log is authoritative on re-attach: `createApp` re-derives `asButler` from the copied/restored `Session.Created` role, so fork-attach and restart bring the coordinator toolset back instead of leaving a butler-badged session with a plain toolset.
- **Per-session permission level is durable** (`runtime/app.ts`): `setPolicy` logs `Session.PolicyChanged {from,to,by:"host"}`; `createApp` folds the LAST such event into `currentPolicy` on (re)creation. Rationale: a restart must never silently WIDEN a restricted session (readonly → strict would be a privilege escalation by crash). The transport exposes GET/POST `/v1/session/:id/policy`; fork copies the events, so a fork inherits the source's level. The client renders it as a per-session menu; the settings-page value remains the DEFAULT for new sessions.
- **Evidence**: `config.test.ts` (preset upsert/redaction, activation, env override, removal incl. last-preset, dangling active), `app.test.ts` (butler role logging + body injection + re-attach restore; policy restore across re-create), `server.test.ts` (butler create → registry role; policy endpoint; fork preserves location+role; commands list/expand 404).
