...
- **Tests**: loop.test.ts — model-relative fire (small window folds where the 80k default would not), fallback no-fire (same history, unknown window), explicit-threshold precedence, `compactLimit` derivation table; compaction.test.ts — fold/projection/summarizer-fallback semantics.
