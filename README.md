...
The officially exportable CLI targets are `linux-x64`, `windows-x64`, and `windows-x64-baseline`. `linux-x64` has been produced and runtime-verified on a Linux host (`nh --version` answers the package version and `nh setup profile --help` runs); `windows-x64` has been cross-compiled and runtime-verified on a Windows runner via the `export-cli` workflow's `validate-windows` job (`nh.exe` answers the version and setup help); `windows-x64-baseline` has a cross-compiled, structure-checked artifact with its Windows-runner validation pending. Outputs are written to `packages/opencode/dist/exports/` as a ZIP, SHA-256 checksum, and manifest. The export path does not publish npm packages, create a GitHub Release, push containers, or create tags.
