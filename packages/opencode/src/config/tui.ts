...
import { isRecord } from "@newhorse/tui/util/record"
...
import { TuiKeybind } from "@newhorse/tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@newhorse/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: {
  ...
  // 4. Config directories (and OPENCODE_CONFIG_DIR) discovered while walking up
  // the tree. Also returned below so callers can install plugin dependencies
  // from each location.
  const dirs = unique(directories).filter((dir) => ConfigPaths.isConfigDir(dir) || dir === Flag.OPENCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!ConfigPaths.isConfigDir(dir) && dir !== Flag.OPENCODE_CONFIG_DIR) continue
