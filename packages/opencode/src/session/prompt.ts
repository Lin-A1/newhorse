...
import { MAX_STEPS_PROMPT } from "@newhorse/core/session/runner/max-steps"
...
import { LLM } from "./llm"
import { Shell } from "@newhorse/core/shell"
...
export class Service extends Context.Service<Service, Interface>()("@newhorse/SessionPrompt") {}
  ...
  Effect.gen(function* () {
    ...
    const handleSubtask = Effect.fn("SessionPrompt.handleSubtask")(function* (input: {
      ...
      const result = yield* taskTool
        .execute(taskArgs, {
          ...
          callID: part.callID,
          extra: { bypassAgentCheck: [task.agent], promptOps },
          messages: msgs,
          ...
          ask: (req: any) =>
            permission
              .ask({
                ...req,
                sessionID,
                ruleset: Agent.effectivePermission(taskAgent, session.permission ?? []),
              })
              .pipe(Effect.orDie),
        })
        .pipe(
    ...
    const createUserMessage = Effect.fn("SessionPrompt.createUserMessage")(function* (input: PromptInput) {
      ...
      const resolvePart: (part: PromptInput["parts"][number]) => Effect.Effect<Draft<SessionV1.Part>[]> = Effect.fn(
        ...
        if (part.type === "agent") {
          const perm = Permission.evaluate("task", part.name, Agent.effectivePermission(ag, current.permission ?? []))
      ...
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            const bypassAgentCheck = lastUserMsg?.parts.filter((part) => part.type === "agent").map((part) => part.name)
            const promptOps = yield* ops()
