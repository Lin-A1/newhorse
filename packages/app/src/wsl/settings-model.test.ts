...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when newhorse needs attention", () => {
    ...
    ).toBe("Install newhorse")
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/nh",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("Update newhorse")
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/nh",
  ...
  test("does not report ready when newhorse is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        opencodeChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.newhorse/bin/nh",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
