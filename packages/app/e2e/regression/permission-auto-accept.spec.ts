...
async function connect(page: Page, transport: ReturnType<typeof installSseTransport>) {
...
test("auto-responds to a pending permission when config permission is allow", async ({ page }) => {
  const { transport } = await setupMock(page)
  ...
  await transport.send(permissionEvent)
  await reply
...
test("auto-responds when the directory auto-accept toggle is on", async ({ page }) => {
  // Directory auto-accept is persisted in the permission store keyed by
  // `${base64(directory)}/*` — the same key the settings toggle writes.
  await page.addInitScript(({ key }) => {
    localStorage.setItem("opencode.global.dat:permission", JSON.stringify({ autoAccept: { [key]: true } }))
  }, { key: `${base64Encode(directory)}/*` })
