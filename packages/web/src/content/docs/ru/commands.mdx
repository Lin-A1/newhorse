...
Создайте `.newhorse/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию newhorse или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем newhorse [config](/docs/config):
...
- Глобальный: `~/.config/newhorse/commands/`
- Для каждого проекта: `.newhorse/commands/`
...
newhorse включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
