...
Il existe deux types d'agents dans Newhorse ; agents primaires et sous-agents.
...
Newhorse est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Newhorse est livré avec trois sous-agents intégrés, **General**, **Explore** et **Scout**. Nous les verrons ci-dessous.

---

## Agents intégrés

Newhorse est livré avec deux agents primaires intégrés et trois sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `newhorse.json` :
...
Si aucune température n'est spécifiée, Newhorse utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Newhorse et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Newhorse utilise le format `provider/model-id`. Par exemple, si vous utilisez [Newhorse Zen](/docs/zen), vous utiliserez `newhorse/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `newhorse models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/Lin-A1/newhorse).
:::
