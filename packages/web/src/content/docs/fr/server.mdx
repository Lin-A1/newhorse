---
title: Serveur
description: Interagissez avec le serveur newhorse via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `newhorse serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client newhorse peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `newhorse.local` |
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `newhorse`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `newhorse serve` et à `newhorse web`.
...
Lorsque vous exécutez `newhorse`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur newhorse pour interagir avec newhorse par programmation.
:::

Cette architecture permet à newhorse de prendre en charge plusieurs clients et vous permet d'interagir avec newhorse par programme.

Vous pouvez exécuter `newhorse serve` pour démarrer un serveur autonome. Si vous avez le
newhorse TUI en cours d'exécution, `newhorse serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Newhorse [IDE](/docs/ide).
...
Le serveur newhorse expose les API suivantes.
