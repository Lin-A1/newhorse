---
title: Plugins
description: Écrivez vos propres plugins pour étendre Newhorse.
---

Les plugins vous permettent d'étendre Newhorse en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Newhorse.
...
- `.newhorse/plugins/` - Plugins au niveau du projet
- `~/.config/newhorse/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/newhorse/node_modules/`.
...
1. Configuration globale (`~/.config/newhorse/newhorse.json`)
2. Configuration du projet (`newhorse.json`)
3. Répertoire global des plugins (`~/.config/newhorse/plugins/`)
4. Répertoire des plugins du projet (`.newhorse/plugins/`)
...
Newhorse exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK newhorse pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre newhorse.
...
:::note
Si vous utilisez l'application de bureau Newhorse, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez newhorse de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à newhorse :
...
L'assistant `tool` crée un outil personnalisé que newhorse peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour newhorse aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://github.com/Lin-A1/newhorse/docs/sdk) pour plus de détails.
