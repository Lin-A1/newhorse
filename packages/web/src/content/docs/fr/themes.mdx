...
Avec Newhorse, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Newhorse utilise notre propre thème `newhorse`.
...
Newhorse est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Newhorse corresponde à l'apparence de leur terminal
...
Newhorse prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/newhorse/themes/*.json` ou `$XDG_CONFIG_HOME/newhorse/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.newhorse/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.newhorse/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/newhorse/themes
vim ~/.config/newhorse/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .newhorse/themes
vim .newhorse/themes/my-theme.json
```
...
{
  "$schema": "https://github.com/Lin-A1/newhorse/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
