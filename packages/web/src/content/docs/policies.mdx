---
title: Policies
description: Control which configured resources Newhorse may use.
---

Policies control whether Newhorse may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `newhorse.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether Newhorse may use a resource such as an LLM provider.
...
Newhorse currently supports one policy action:
