---
title: CLI
description: Opcje i polecenia wiersza poleceń Newhorse.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI Newhorse domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
newhorse
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać Newhorse programowo.
...
Uruchamia interfejs terminalowy (TUI) Newhorse.
...
Interfejs CLI Newhorse zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami Newhorse.
...
Dołącz terminal do już działającego serwera backendu Newhorse uruchomionego za pomocą `serve` lub `web`.

```bash
newhorse attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem Newhorse. Na przykład:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `newhorse`) |
...
Newhorse integruje się z wieloma dostawcami. Możesz użyć `newhorse auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/newhorse/auth.json`.

```bash
newhorse auth login
```

Podczas uruchamiania Newhorse ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w Newhorse.
...
Uruchom Newhorse w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `newhorse serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Dołącz do działającego serwera Newhorse (np. http://localhost:4096)                                   |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `newhorse`) |
...
Uruchom serwer Newhorse (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
newhorse serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API Newhorse bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `newhorse`).
...
Lista wszystkich sesji Newhorse.
...
Pokaż statystyki wykorzystania tokenów i sesji Newhorse.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania Newhorse.
...
Uruchom serwer Newhorse z interfejsem internetowym.

```bash
newhorse web
```

Uruchamia to serwer HTTP i udostępnia Newhorse przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `newhorse`).
...
Odinstaluj Newhorse i usuń wszystkie powiązane pliki.
...
Aktualizuj Newhorse do najnowszej lub określonej wersji.
...
Interfejs CLI Newhorse przyjmuje następujące flagi globalne dla każdego polecenia.
...
Newhorse można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `newhorse`)    |
