---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć newhorse.
---

Wtyczki umożliwiające wyłączenie newhorse poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania newhorse.
...
- `.newhorse/plugins/` - Wtyczki na poziomie projektu
- `~/.config/newhorse/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/newhorse/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/newhorse/newhorse.json`)
2. Konfiguracja projektu (`newhorse.json`)
3. Globalny katalog wtyczek (`~/.config/newhorse/plugins/`)
4. Katalog wtyczek projektu (`.newhorse/plugins/`)
...
newhorse uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z newhorse do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do newhorse.
...
:::note
Jeśli korzystasz z aplikacji komputerowej newhorse, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj newhorse czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do newhorse:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod newhorse. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://github.com/Lin-A1/newhorse/docs/sdk).
