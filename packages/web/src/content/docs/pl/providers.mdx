---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w newhorse.
---

import config from "../../../../config.mjs"
export const console = config.console

newhorse używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji newhorse.
...
w `~/.local/share/newhorse/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim newhorse
config.
...
Newhorse Zen to lista modeli dostarczonych przez zespół newhorse, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z newhorse. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Newhorse Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Newhorse Zen` i przejdź do [newhorse.ai/auth](https://github.com/Lin-A1/newhorse/zen).
...
Działa jak każdy inny dostawca w newhorse i jest całkowicie opcjonalny w użyciu.

---

## Newhorse Go

Newhorse Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół newhorse, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z newhorse.

1. Uruchom polecenie `/connect` w TUI, wybierz `Newhorse Go` i przejdź do [newhorse.ai/auth](https://github.com/Lin-A1/newhorse/zen).
...
Aby używać Amazon Bedrock z newhorse:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania newhorse:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `newhorse.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w newhorse nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować newhorse do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod newhorse działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji newhorse.
...
newhorse używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować newhorse
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`newhorse.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [newhorse-gitlab-auth](https://www.npmjs.com/package/newhorse-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem newhorse:
...
Aby używać Google Vertex AI z newhorse:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania newhorse.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą newhorse, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [newhorse-helicone-session](https://github.com/H2Shami/newhorse-helicone-session), aby automatycznie rejestrować każdą konwersację newhorse jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja newhorse będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować newhorse tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować newhorse tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować newhorse tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem newhorse. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/newhorse).
:::
...
Aby korzystać z Ollama Cloud z newhorse:
...
3. Skopiuj klucz API do użycia w newhorse.
...
6. **Ważne**: Przed użyciem modeli chmurowych w newhorse musisz pobrać informacje o modelu lokalnie:
...
Newhorse Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół newhorse. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Newhorse Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Newhorse Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API newhorse.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji newhorse.
...
5. Możesz je także dostosować za pomocą konfiguracji newhorse. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z newhorse:
...
Możesz także dostosować modele za pomocą konfiguracji newhorse. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z newhorse. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `newhorse.json` w katalogu projektu:
...
Pola `limit` pozwalają newhorse zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `newhorse auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację newhorse i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji newhorse.
