...
Aby debugować problemy z newhorse, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\newhorse\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `newhorse --log-level DEBUG`.

---

## Przechowywanie danych

newhorse przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:
...
newhorse Desktop uruchamia w tle lokalny serwer newhorse (sidecar `newhorse-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Newhorse` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/newhorse/newhorse.jsonc` (lub `~/.config/newhorse/newhorse.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/newhorse/newhorse.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\newhorse\newhorse.jsonc`
...
newhorse może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby newhorse mógł ją odbudować.

1. Całkowicie zamknij newhorse Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/newhorse`
- **Linux**: usuń `~/.cache/newhorse` (lub uruchom `rm -rf ~/.cache/newhorse`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\newhorse`

3. Uruchom ponownie newhorse Desktop.

---

### Napraw problemy z połączeniem z serwerem

newhorse Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `newhorse.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
W systemie Windows newhorse Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami newhorse.

---

### Brak powiadomień

newhorse Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla newhorse w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie newhorse Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji newhorse Desktop):

- `newhorse.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `newhorse.global.dat` i `newhorse.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z newhorse:
...
3. Upewnij się, że masz najnowszą wersję z `newhorse upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `newhorse models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\newhorse`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. newhorse dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\newhorse`

2. Uruchom ponownie kod newhorse, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to newhorse do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
newhorse wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
