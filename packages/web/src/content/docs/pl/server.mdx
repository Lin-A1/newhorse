---
title: Serwer
description: Interakcja z serwerem newhorse za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `newhorse serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient newhorse.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `newhorse.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `newhorse` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `newhorse serve`, jak i `newhorse web`.
...
Kiedy uruchomisz `newhorse`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera newhorse do programowej interakcji z newhorse.
:::

Ta architektura umożliwia obsługę wielu klientów przez newhorse i programową interakcję z kodem otwartym.

Możesz uruchomić `newhorse serve`, aby uruchomić samodzielny serwer. Jeśli masz
newhorse TUI działa, `newhorse serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki newhorse [IDE](/docs/ide).
...
Serwer newhorse udostępnia następujące interfejsy API.
