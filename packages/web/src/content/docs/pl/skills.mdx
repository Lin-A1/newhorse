...
Umiejętności agenta pozwalają newhorse odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
newhorse przeszukuje te lokalizacje:

- Project config: `.newhorse/skills/<name>/SKILL.md`
- Global config: `~/.config/newhorse/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu newhorse przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.newhorse/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/newhorse/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.newhorse/skills/git-release/SKILL.md` w ten sposób:
...
newhorse wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `newhorse.json`:
...
**Dla agentów wbudowanych** (w `newhorse.json`):
