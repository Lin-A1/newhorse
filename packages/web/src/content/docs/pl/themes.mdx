...
Dzięki newhorse możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie newhorse używa naszego własnego motywu `newhorse`.
...
newhorse ma kilka wbudowanych motywów.
...
- Chcą, aby Newhorse pasował do wyglądu ich terminala
...
newhorse obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/newhorse/themes/*.json` lub `$XDG_CONFIG_HOME/newhorse/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.newhorse/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.newhorse/themes/*.json`
