...
With Newhorse you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Newhorse uses our own `newhorse` theme.
...
Newhorse comes with several built-in themes.
...
- Want Newhorse to match their terminal's appearance
...
Newhorse supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/newhorse/themes/*.json` or `$XDG_CONFIG_HOME/newhorse/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.newhorse/themes/*.json`
4. **Current working directory** - Defined in `./.newhorse/themes/*.json`
