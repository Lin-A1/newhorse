---
title: CLI
description: newhorse CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

newhorse CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
newhorse
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, newhorse ile programlı olarak etkileşim kurmanıza olanak tanır.
...
newhorse terminal kullanıcı arayüzünü başlatır.
...
newhorse CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

newhorse için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir newhorse arka uç sunucusuna bir terminal ekleyin.

```bash
newhorse attach [url]
```

Bu, TUI öğesinin uzak bir newhorse arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `newhorse`) |
...
newhorse, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `newhorse auth login` kullanabilirsiniz. Bu `~/.local/share/newhorse/auth.json`'de saklanıyor.

```bash
newhorse auth login
```

newhorse başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları newhorse'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek newhorse'u etkileşimli olmayan modda çalıştırın.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `newhorse serve` örneğine de ekleyebilirsiniz:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Çalışan bir newhorse sunucusuna ekleyin (ör. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Temel kimlik doğrulama parolası (varsayılan: `OPENCODE_SERVER_PASSWORD`)                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `newhorse`) |
...
API erişimi için headless bir newhorse sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
newhorse serve
```

Bu, TUI arayüzü olmadan newhorse işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `newhorse` şeklindedir).
...
Tüm newhorse oturumlarını listeleyin.
...
newhorse oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya newhorse paylaşımından URL oturum verilerini içe aktarın.

```bash
newhorse import <file>
```

Yerel bir dosyadan veya newhorse paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir newhorse sunucusu başlatın.

```bash
newhorse web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla newhorse'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `newhorse` şeklindedir).
...
newhorse'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
newhorse'u en son sürüme veya belirli bir sürüme günceller.
...
newhorse CLI aşağıdaki global bayrakları destekler.
...
newhorse ortam değişkenleri kullanılarak yapılandırılabilir.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Temel kimlik doğrulama kullanıcı adını geçersiz kıl (varsayılan `newhorse`) |
