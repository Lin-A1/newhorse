---
title: ACP Desteği
description: newhorse'u ACP uyumlu herhangi bir editörde kullanın.
---

newhorse, [Agent Client Protocol](https://agentclientprotocol.com) veya (ACP) dosyalarını destekleyerek onu doğrudan uyumlu editörlerde ve IDE'lerde kullanmanızı sağlar.
...
newhorse'u ACP aracılığıyla kullanmak için düzenleyicinizi `newhorse acp` komutunu çalıştıracak şekilde yapılandırın.

Komut, newhorse'u, editörünüzle stdio aracılığıyla JSON-RPC üzerinden iletişim kuran ACP uyumlu bir alt süreç olarak başlatır.
...
Açmak için AI Chat agent seçicisindeki yeni 'Newhorse' ajanını kullanın.
...
newhorse'u [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)'de ACP ajanı olarak kullanmak için Neovim yapılandırmanıza aşağıdakileri ekleyin:
...
Bu yapılandırma, CodeCompanion'ı newhorse'u sohbet için ACP ajanı olarak kullanacak şekilde ayarlar.
...
newhorse, terminalde olduğu gibi ACP aracılığıyla aynı şekilde çalışır. Tüm özellikler desteklenir:
...
- newhorse yapılandırmanızda yapılandırılmış MCP sunucuları
