...
newhorse ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak newhorse kendi `newhorse` temasını kullanır.
...
newhorse birden fazla yerleşik temayla gelir.
...
- newhorse'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
newhorse, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/newhorse/themes/*.json` veya `$XDG_CONFIG_HOME/newhorse/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.newhorse/themes/*.json`
4. **Geçerli çalışma dizini** - `./.newhorse/themes/*.json`
