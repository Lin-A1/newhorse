...
Newhorse Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Newhorse'daki diğer sağlayıcılar gibi çalışır. Newhorse Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Newhorse'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Newhorse Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Newhorse Go, Newhorse'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Newhorse Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Newhorse Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Newhorse Go'ya abone olabilir.
:::
...
Newhorse Go aşağıdaki limitleri içerir:
...
Newhorse yapılandırmanızdaki [model id](/docs/config/#models) formatı `newhorse-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `newhorse-go/kimi-k3` kullanmalısınız.
...
Newhorse Go'yu şu amaçlarla oluşturduk:
...
4. Newhorse ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
