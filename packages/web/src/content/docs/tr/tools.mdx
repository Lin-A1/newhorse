...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. newhorse yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
newhorse içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca newhorse provider kullanılırken veya `OPENCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

newhorse başlatırken etkinleştirmek için:
