---
title: Eklentiler
description: Newhorse'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Newhorse'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Newhorse'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.newhorse/plugins/` - Proje düzeyinde eklentiler
- `~/.config/newhorse/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/newhorse/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/newhorse/newhorse.json`)
2. Project config (`newhorse.json`)
3. Global eklenti dizini (`~/.config/newhorse/plugins/`)
4. Proje eklenti dizini (`.newhorse/plugins/`)
...
Newhorse bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Newhorse SDK istemcisi.
...
Newhorse'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Newhorse masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Newhorse'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Newhorse'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Newhorse'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Newhorse'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://github.com/Lin-A1/newhorse/docs/sdk) bakın.
