---
title: Plugins
description: Skriv dine egne plugins for at udvide Newhorse.
---

Plugins giver dig mulighed for at udvide Newhorse ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Newhorses standardadfærd.
...
- `.newhorse/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/newhorse/node_modules/`.
...
1. Global konfiguration (`~/.config/newhorse/newhorse.json`)
2. Projektkonfiguration (`newhorse.json`)
3. Global plugin-mappe (`~/.config/newhorse/plugins/`)
4. Projekt plugin bibliotek (`.newhorse/plugins/`)
...
Newhorse kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En newhorse SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide newhorse.
...
:::note
Hvis du bruger Newhorse desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå newhorse i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til newhorse:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som newhorse kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for newhorse sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://github.com/Lin-A1/newhorse/docs/sdk) for detaljer.
