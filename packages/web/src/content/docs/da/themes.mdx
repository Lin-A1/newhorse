...
Med Newhorse kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Newhorse vårt eget `newhorse`-tema.
...
Newhorse støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/newhorse/themes/*.json` eller `$XDG_CONFIG_HOME/newhorse/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.newhorse/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.newhorse/themes/*.json`
