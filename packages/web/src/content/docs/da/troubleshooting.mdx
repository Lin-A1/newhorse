...
For at fejlfinde problemer med Newhorse, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\newhorse\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `newhorse --log-level DEBUG`.

---

## Lagring

newhorse gemmer sessionsdata og andre applikationsdata på disken på:
...
Newhorse Desktop kører en lokal Newhorse-server (`newhorse-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Newhorse`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/newhorse/newhorse.jsonc` (eller `~/.config/newhorse/newhorse.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/newhorse/newhorse.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\newhorse\newhorse.jsonc`
...
Newhorse kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Newhorse kan genopbygge den.

1. Afslut Newhorse Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/newhorse`
- **Linux**: slet `~/.cache/newhorse` (eller kør `rm -rf ~/.cache/newhorse`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\newhorse`

3. Start Newhorse Desktop på ny.

---

### Løs problemer med serverforbindelse

Newhorse Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `newhorse.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver Newhorse Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Newhorse.

---

### Meddelelser vises ikke

Newhorse Desktop viser kun systemvarsler når:

- varsler er aktiveret for Newhorse i dine OS-indstillinger, og
...
1. Afslut Newhorse Desktop.
2. Find og slet disse filer (de findes i Newhorse Desktop-appens datamappe):

- `newhorse.settings.dat` (skrivebordsstandardserver URL)
- `newhorse.global.dat` og `newhorse.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Newhorse:
...
For at finde ud af hvilke modeller du har adgang til, kør `newhorse models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\newhorse`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. newhorse installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\newhorse`

2. Start newhorse på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge newhorse til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
newhorse vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
