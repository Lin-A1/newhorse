---
title: CLI
description: Newhorse CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Newhorse CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
newhorse
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med Newhorse programmatisk.
...
Start Newhorse terminalbrugergrænsefladen.
...
Newhorse CLI har også følgende kommandoer.

---

### agent

Administrer agenter i Newhorse.
...
Tilslut en terminal til en allerede kørende Newhorse backend-server startet via `serve` eller `web` kommandoer.

```bash
newhorse attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern Newhorse backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `newhorse`) |
...
Newhorse drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `newhorse auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/newhorse/auth.json`.

```bash
newhorse auth login
```

Når Newhorse starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i Newhorse.
...
Kør newhorse i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `newhorse serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Tilslut til en kørende newhorse-server (f.eks. http://localhost:4096)               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `OPENCODE_SERVER_PASSWORD`)                   |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `newhorse`)   |
...
Start en hovedløs Newhorse-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
newhorse serve
```

Dette starter en HTTP-server, der giver API-adgang til newhorse-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `newhorse`).
...
Administrator Newhorse sessionsholder.
...
Liste over alle Newhorse sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine Newhorse-sessioner.
...
Importer sessionsdata fra en JSON fil eller Newhorse del URL.

```bash
newhorse import <file>
```

Du kan importere fra en lokal fil eller en Newhorse share URL.
...
Start en hovedløs Newhorse-server med en webgrænseflade.

```bash
newhorse web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til Newhorse via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `newhorse`).
...
Afinstaller Newhorse og fjern alle relaterede filer.
...
Opdaterer Newhorse til den nyeste version eller en specifik version.
...
newhorse CLI tager følgende globale flag.
...
Newhorse kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `newhorse`) |
