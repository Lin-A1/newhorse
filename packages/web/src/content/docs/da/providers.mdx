---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Newhorse.
---

import config from "../../../../config.mjs"
export const console = config.console

Newhorse bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Newhorse-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/newhorse/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Newhorse-konfigurationen.
...
Newhorse Zen er en liste over modeller leveret af Newhorse-teamet, som er blevet testet og verificeret til at fungere godt med Newhorse. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Newhorse Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg newhorse og gå til [newhorse.ai/auth](https://github.com/Lin-A1/newhorse/auth).
...
Det fungerer som alle andre udbydere i Newhorse og er helt valgfrit at bruge.

---

## Newhorse Go

Newhorse Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Newhorse-teamet, som er testet og verificeret til at fungere godt med Newhorse.

1. Kør kommandoen `/connect` i TUI, vælg `Newhorse Go`, og gå til [newhorse.ai/auth](https://github.com/Lin-A1/newhorse/zen).
...
Sådan bruger du Amazon Bedrock med Newhorse:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører newhorse:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `newhorse.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Newhorse understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere newhorse til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at newhorse skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem newhorse-konfigurationen.
...
gemme tokenet i newhorse auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Newhorse bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Newhorse
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`newhorse.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [newhorse-gitlab-auth](https://www.npmjs.com/package/newhorse-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `newhorse.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Newhorse:
...
Sådan bruger du Google Vertex AI med Newhorse:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører newhorse.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem newhorse, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [newhorse-helicone-session](https://github.com/H2Shami/newhorse-helicone-session) plugin for automatisk at logge hver Newhorse-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Newhorse-samtale opført som en separat session.
...
Du kan konfigurere newhorse til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere newhorse til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere newhorse til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Newhorse. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/newhorse) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Newhorse:
...
3. Kopier API-nøglen til brug i Newhorse.
...
6. **Vigtigt**: Før du bruger skymodeller i Newhorse, skal du hente modelinformationen lokalt:
...
Newhorse Zen er en liste over testede og verificerede modeller leveret af Newhorse-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Newhorse Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Newhorse Zen**.

   ```txt
   /connect
   ```

3. Indtast Newhorse API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem newhorse-konfigurationen.
...
5. Du kan også tilpasse dem gennem newhorse-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Newhorse:
...
Du kan også tilpasse modeller gennem newhorse-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med newhorse. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `newhorse.json`-filen i projektkataloget:
...
`limit`-felterne lader Newhorse forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `newhorse auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek newhorse-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i newhorse-konfigurationen.
