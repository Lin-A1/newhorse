---
title: Config
description: Using the Newhorse JSON config.
---

You can configure Newhorse using a JSON config file.

---

## Format

Newhorse supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="newhorse.jsonc"
{
...
1. **Remote config** (from `.well-known/newhorse`) - organizational defaults
2. **Global config** (`~/.config/newhorse/newhorse.json`) - user preferences
3. **Custom config** (`OPENCODE_CONFIG` env var) - custom overrides
4. **Project config** (`newhorse.json` in project) - project-specific settings
5. **`.newhorse` directories** - agents, commands, plugins
6. **Inline config** (`OPENCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/newhorse/` on macOS) - admin-controlled
...
:::note
The `.newhorse` and `~/.config/newhorse` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/newhorse` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/newhorse"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Newhorse config in `~/.config/newhorse/newhorse.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/newhorse/tui.json`.
...
Add `newhorse.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Newhorse starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
modes, and plugins just like the standard `.newhorse` directory, and should
...
The custom directory is loaded after the global config and `.newhorse` directories, so it **can override** their settings.
...
Drop an `newhorse.json` or `newhorse.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/newhorse/` |
| Linux    | `/etc/newhorse/`                         |
| Windows  | `%ProgramData%\newhorse`                 |
...
On macOS, Newhorse reads managed preferences from the `ai.newhorse.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Newhorse checks these paths:

1. `/Library/Managed Preferences/<user>/ai.newhorse.managed.plist`
2. `/Library/Managed Preferences/ai.newhorse.managed.plist`

The plist keys map directly to `newhorse.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.newhorse.managed` PayloadType. The Newhorse config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.newhorse.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.newhorse.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.newhorse</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`newhorse.ai/config.json`**](https://github.com/Lin-A1/newhorse/config.json).

TUI config uses [**`newhorse.ai/tui.json`**](https://github.com/Lin-A1/newhorse/tui.json).
...
```json title="tui.json"
{
  "$schema": "https://github.com/Lin-A1/newhorse/tui.json",
  "scroll_speed": 3,
  "scroll_acceleration": {
    "enabled": true
  },
...
Legacy `theme`, `keybinds`, and `tui` keys in `newhorse.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `newhorse serve` and `newhorse web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Newhorse server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `newhorse.local`. Useful for running multiple instances on the same network.
...
If not specified, Newhorse will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Newhorse config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Newhorse tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Newhorse actions on configured resources. Currently, policies can control which providers Newhorse may use.
...
Newhorse normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Newhorse omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/newhorse/agents/` or `.newhorse/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Newhorse will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`newhorse run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/newhorse/commands/` or `.newhorse/commands/`. [Learn more here](/docs/commands).
...
Newhorse uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Newhorse will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, newhorse **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Newhorse with custom tools, hooks, and integrations.

Place plugin files in `.newhorse/plugins/` or `~/.config/newhorse/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="newhorse.json"
{
  "$schema": "https://github.com/Lin-A1/newhorse/config.json",
  "plugin": ["newhorse-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict Newhorse to only use specific providers rather than disabling them one by one.
