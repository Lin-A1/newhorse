...
يمكنك إضافة أدوات خارجية إلى Newhorse باستخدام _Model Context Protocol_، أو MCP. يدعم Newhorse خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Newhorse](https://github.com/Lin-A1/newhorse/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/newhorse` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Newhorse تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Newhorse بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Newhorse المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `newhorse mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Newhorse الرموز بشكل آمن في `~/.local/share/newhorse/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Newhorse إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Newhorse مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Newhorse بحساب Sentry الخاص بك.
