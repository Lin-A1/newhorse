---
title: المزوّدون
description: استخدام أي موفّر LLM في Newhorse.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Newhorse ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Newhorse.
...
في `~/.local/share/newhorse/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Newhorse.
...
Newhorse Zen هي قائمة نماذج يوفّرها فريق Newhorse وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Newhorse. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Newhorse Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر newhorse، ثم انتقل إلى [newhorse.ai/auth](https://github.com/Lin-A1/newhorse/auth).
...
يعمل مثل أي مزوّد آخر في Newhorse واستخدامه اختياري بالكامل.

---

## Newhorse Go

Newhorse Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Newhorse، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Newhorse.

1. شغّل الأمر `/connect` في TUI، واختر `Newhorse Go`، ثم انتقل إلى [newhorse.ai/auth](https://github.com/Lin-A1/newhorse/zen).
...
لاستخدام Amazon Bedrock مع Newhorse:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل newhorse:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `newhorse.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Newhorse غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين newhorse لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل newhorse بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات newhorse.
...
تخزين الرمز في مساحة تخزين مصادقة newhorse.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Newhorse نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Newhorse
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`newhorse.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [newhorse-gitlab-auth](https://www.npmjs.com/package/newhorse-gitlab-auth).

##### التهيئة

خصّص عبر `newhorse.json`:
...
لاستخدام اشتراك GitHub Copilot مع newhorse:
...
لاستخدام Google Vertex AI مع Newhorse:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر newhorse، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [newhorse-helicone-session](https://github.com/H2Shami/newhorse-helicone-session) لتسجيل كل محادثة Newhorse تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Newhorse مدرجة كجلسة مستقلة.
...
يمكنك ضبط newhorse لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط newhorse لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط newhorse لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Newhorse. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/newhorse) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Newhorse:
...
3. انسخ مفتاح API لاستخدامه في Newhorse.
...
6. **مهم**: قبل استخدام نماذج السحابة في Newhorse، يجب سحب معلومات النموذج محليا:
...
Newhorse Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Newhorse. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Newhorse Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Newhorse Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Newhorse.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات newhorse.
...
5. يمكنك أيضا تخصيصها عبر إعدادات newhorse. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Newhorse:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات newhorse. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع newhorse. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `newhorse.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Newhorse بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `newhorse auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات newhorse وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات newhorse.
