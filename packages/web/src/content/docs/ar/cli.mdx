---
title: CLI
description: أوامر وخيارات CLI في Newhorse.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ Newhorse افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
newhorse
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع Newhorse برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ Newhorse.
...
يتضمن CLI الخاص بـ Newhorse أيضا الأوامر التالية.

---

### agent

إدارة وكلاء Newhorse.
...
إرفاق terminal بخادم الواجهة الخلفية لـ Newhorse قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
newhorse attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ Newhorse تعمل عن بعد. على سبيل المثال:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `newhorse`) |
...
يعتمد Newhorse على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `newhorse auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/newhorse/auth.json`.

```bash
newhorse auth login
```

عند تشغيل Newhorse يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في Newhorse.
...
تشغيل newhorse في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `newhorse serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| <nobr><code>{"--attach"}</code></nobr>   |         | الإرفاق بخادم newhorse قيد التشغيل (مثل http://localhost:4096)                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`    | كلمة مرور المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_PASSWORD`)                |
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `newhorse`) |
...
بدء خادم Newhorse بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
newhorse serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف newhorse دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `newhorse`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات Newhorse لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة Newhorse.

```bash
newhorse import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة Newhorse.
...
بدء خادم Newhorse بدون واجهة مع واجهة ويب.

```bash
newhorse web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى Newhorse عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `newhorse`).
...
إلغاء تثبيت Newhorse وإزالة جميع الملفات ذات الصلة.
...
تحديث newhorse إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر newhorse الخيارات العامة التالية.
...
يمكن تهيئة Newhorse باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `newhorse`) |
