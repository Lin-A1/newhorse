---
title: Plugins
description: Escreva seus próprios plugins para estender o newhorse.
---

Plugins permitem que você estenda o newhorse conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do newhorse.
...
- `.newhorse/plugins/` - Plugins em nível de projeto
- `~/.config/newhorse/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/newhorse/node_modules/`.
...
1. Configuração global (`~/.config/newhorse/newhorse.json`)
2. Configuração do projeto (`newhorse.json`)
3. Diretório de plugins global (`~/.config/newhorse/plugins/`)
4. Diretório de plugins do projeto (`.newhorse/plugins/`)
...
O newhorse executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do newhorse para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o newhorse.
...
:::note
Se você estiver usando o aplicativo desktop newhorse, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o newhorse de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao newhorse:
...
O helper `tool` cria uma ferramenta personalizada que o newhorse pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o newhorse junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://github.com/Lin-A1/newhorse/docs/sdk) para detalhes.
