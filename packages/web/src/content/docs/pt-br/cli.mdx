---
title: CLI
description: Opções e comandos da CLI do newhorse.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do newhorse, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
newhorse
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o newhorse programaticamente.
...
Inicie a interface de usuário do terminal do newhorse.
...
A CLI do newhorse também possui os seguintes comandos.

---

### agent

Gerencie agentes para o newhorse.
...
Anexe um terminal a um servidor backend do newhorse já em execução, iniciado via comandos `serve` ou `web`.

```bash
newhorse attach [url]
```

Isso permite usar o TUI com um backend newhorse remoto. Por exemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `newhorse`) |
...
O newhorse é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `newhorse auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/newhorse/auth.json`.

```bash
newhorse auth login
```

Quando o newhorse é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no newhorse.
...
Execute o newhorse em modo não interativo passando um prompt diretamente.
...
```bash "newhorse run"
newhorse run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `newhorse serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
newhorse run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor newhorse em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `newhorse`) |
...
Inicie um servidor newhorse sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
newhorse serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do newhorse sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `newhorse`).
...
Liste todas as sessões do newhorse.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do newhorse.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do newhorse.

```bash
newhorse import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do newhorse.
...
Inicie um servidor newhorse sem cabeça com uma interface web.

```bash
newhorse web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o newhorse através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `newhorse`).
...
Desinstale o newhorse e remova todos os arquivos relacionados.
...
Atualiza o newhorse para a versão mais recente ou uma versão específica.
...
A CLI do newhorse aceita as seguintes flags globais.
...
O newhorse pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `newhorse`) |
