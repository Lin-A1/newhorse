...
Existem dois tipos de agentes no newhorse; agentes primários e subagentes.
...
newhorse vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Newhorse vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Newhorse vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Newhorse, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `newhorse.json`:
...
- Global: `~/.config/newhorse/agents/`
- Por projeto: `.newhorse/agents/`
...
Se nenhuma temperatura for especificada, o newhorse usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do newhorse quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do newhorse usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Newhorse Zen](/docs/zen), você usaria `newhorse/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `newhorse models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/Lin-A1/newhorse).
:::
