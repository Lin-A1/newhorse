...
As habilidades do agente permitem que o newhorse descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O newhorse pesquisa nesses locais:

- Configuração do projeto: `.newhorse/skills/<name>/SKILL.md`
- Configuração global: `~/.config/newhorse/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o newhorse sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.newhorse/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/newhorse/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.newhorse/skills/git-release/SKILL.md` assim:
...
O newhorse lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `newhorse.json`:
...
**Para agentes embutidos** (em `newhorse.json`):
