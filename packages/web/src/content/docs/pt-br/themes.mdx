...
Com o newhorse, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o newhorse usa nosso próprio tema `newhorse`.
...
O newhorse vem com vários temas embutidos.
...
- Querem que o newhorse corresponda à aparência do seu terminal
...
O newhorse suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/newhorse/themes/*.json` ou `$XDG_CONFIG_HOME/newhorse/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.newhorse/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.newhorse/themes/*.json`
