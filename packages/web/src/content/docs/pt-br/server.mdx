---
title: Servidor
description: Interaja com o servidor newhorse via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `newhorse serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente newhorse pode usar.
...
```bash
newhorse serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `newhorse.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
newhorse serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `newhorse`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `newhorse serve` quanto ao `newhorse web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password newhorse serve
```
...
Quando você executa `newhorse`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor newhorse para interagir com o newhorse programaticamente.
:::

Essa arquitetura permite que o newhorse suporte múltiplos clientes e permite que você interaja com o newhorse programaticamente.

Você pode executar `newhorse serve` para iniciar um servidor autônomo. Se você tiver o TUI do newhorse em execução, `newhorse serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do newhorse [IDE](/docs/ide).
...
O servidor newhorse expõe as seguintes APIs.
