---
title: TUI
description: Using the Newhorse terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Newhorse provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Newhorse starts the TUI for the current directory.
...
When using the Newhorse TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Newhorse. Allows you to select from available providers and add their API keys.
...
Exit Newhorse. _Aliases_: `/quit`, `/q`
...
This is separate from `newhorse.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Newhorse waits after the leader key. Defaults to `2000`.
...
- `sound_pack` - Sound pack ID to use. Defaults to `newhorse.default`.
