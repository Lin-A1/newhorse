---
title: CLI
description: Opzioni e comandi della CLI di Newhorse.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di Newhorse, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
newhorse
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con Newhorse in modo programmatico.
...
Avvia l'interfaccia testuale di Newhorse (interfaccia utente da terminale).
...
La CLI di Newhorse include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per Newhorse.
...
Collega un terminale a un backend server di Newhorse già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
newhorse attach [url]
```

Questo consente di usare la TUI con un backend Newhorse remoto. Per esempio:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `newhorse`) |
...
Newhorse si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `newhorse auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/newhorse/auth.json`.

```bash
newhorse auth login
```

Quando Newhorse si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in Newhorse.
...
Esegui newhorse in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `newhorse serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server newhorse in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `newhorse`) |
...
Avvia un server Newhorse headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
newhorse serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di newhorse senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `newhorse`).
...
Elenca tutte le sessioni Newhorse.
...
Mostra statistiche di utilizzo token e costo per le sessioni Newhorse.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione Newhorse.

```bash
newhorse import <file>
```

Puoi importare da un file locale o da un URL di condivisione Newhorse.
...
Avvia un server Newhorse headless con interfaccia web.

```bash
newhorse web
```

Avvia un server HTTP e apre un browser per accedere a Newhorse tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `newhorse`).
...
Disinstalla Newhorse e rimuove tutti i file correlati.
...
Aggiorna newhorse all'ultima versione o a una versione specifica.
...
La CLI di newhorse accetta i seguenti flag globali.
...
Newhorse può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `newhorse`)     |
