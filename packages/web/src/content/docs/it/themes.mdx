...
Con Newhorse puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, Newhorse usa il tema `newhorse`.
...
Newhorse include diversi temi integrati.
...
- Vuole che Newhorse corrisponda all'aspetto del terminale
...
Newhorse supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/newhorse/themes/*.json` o `$XDG_CONFIG_HOME/newhorse/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.newhorse/themes/*.json`
4. **Directory di lavoro corrente** - in `./.newhorse/themes/*.json`
