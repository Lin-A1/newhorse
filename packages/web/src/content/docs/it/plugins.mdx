---
title: Plugin
description: Scrivi plugin per estendere Newhorse.
---

I plugin ti permettono di estendere Newhorse agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Newhorse.
...
- `.newhorse/plugins/` - plugin a livello progetto
- `~/.config/newhorse/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/newhorse/node_modules/`.
...
1. Config globale (`~/.config/newhorse/newhorse.json`)
2. Config di progetto (`newhorse.json`)
3. Directory plugin globale (`~/.config/newhorse/plugins/`)
4. Directory plugin di progetto (`.newhorse/plugins/`)
...
Newhorse esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di newhorse per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere newhorse.
...
:::note
Se usi l'app desktop di Newhorse, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a newhorse di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a newhorse:
...
L'helper `tool` crea uno strumento personalizzato che newhorse può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in newhorse insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://github.com/Lin-A1/newhorse/docs/sdk) per i dettagli.
