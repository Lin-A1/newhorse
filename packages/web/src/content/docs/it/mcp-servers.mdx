...
Puoi aggiungere strumenti esterni a Newhorse usando il _Model Context Protocol_, o MCP. Newhorse supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Newhorse Config](https://github.com/Lin-A1/newhorse/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/newhorse`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Newhorse gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Newhorse:
...
Se il server richiede autenticazione, Newhorse ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `newhorse mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Newhorse memorizzera' i token in modo sicuro in `~/.local/share/newhorse/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Newhorse insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Newhorse come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Newhorse al tuo account Sentry.
