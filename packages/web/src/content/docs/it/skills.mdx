...
Le skill degli agenti permettono a Newhorse di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Newhorse cerca in queste posizioni:

- Config di progetto: `.newhorse/skills/<name>/SKILL.md`
- Config globale: `~/.config/newhorse/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Newhorse risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.newhorse/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/newhorse/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.newhorse/skills/git-release/SKILL.md` cosi':
...
Newhorse elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `newhorse.json`:
...
**Per agenti integrati** (in `newhorse.json`):
