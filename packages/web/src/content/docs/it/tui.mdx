---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Newhorse.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Newhorse fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Newhorse avvia la TUI per la directory corrente.
...
Quando usi la TUI di Newhorse, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Newhorse. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Newhorse. _Alias_: `/quit`, `/q`
...
Questo è separato da `newhorse.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Newhorse attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `newhorse.default`.
