---
title: CLI
description: Newhorse CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The Newhorse CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
newhorse
```

But it also accepts commands as documented on this page. This allows you to interact with Newhorse programmatically.
...
Start the Newhorse terminal user interface.
...
The Newhorse CLI also has the following commands.

---

### agent

Manage agents for Newhorse.
...
| Flag                                        | Short | Description                                                                                                                                                                                                                |
| ------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <nobr><code>{"--path"}</code></nobr>        |       | Directory to write the agent file to (defaults to global or `.newhorse/agent` based on the prompt)                                                                                                                         |
...
Attach a terminal to an already running Newhorse backend server started via `serve` or `web` commands.

```bash
newhorse attach [url]
```

This allows using the TUI with a remote Newhorse backend. For example:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `newhorse`) |
...
Newhorse is powered by the provider list at [Models.dev](https://models.dev), so you can use `newhorse auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/newhorse/auth.json`.

```bash
newhorse auth login
```

When Newhorse starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in Newhorse.
...
Run newhorse in non-interactive mode by passing a prompt directly.
...
You can also attach to a running `newhorse serve` instance to avoid MCP server cold boot times on every run:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach to a running newhorse server (e.g., http://localhost:4096)          |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Basic auth password (defaults to `OPENCODE_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `newhorse`) |
...
Start a headless Newhorse server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.

```bash
newhorse serve
```

This starts an HTTP server that provides API access to newhorse functionality without the TUI interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `newhorse`).
...
Delete an Newhorse session.

```bash
newhorse session delete <sessionID>
```
...
Show token usage and cost statistics for your Newhorse sessions.
...
Import session data from a JSON file or Newhorse share URL.

```bash
newhorse import <file>
```

You can import from a local file or an Newhorse share URL.
...
Start a headless Newhorse server with a web interface.

```bash
newhorse web
```

This starts an HTTP server and opens a web browser to access Newhorse through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `newhorse`).
...
```bash
newhorse plugin <module>
```

Or use the alias.

```bash
newhorse plug <module>
```
...
Fetch and checkout a GitHub PR branch, then run Newhorse.
...
Uninstall Newhorse and remove all related files.
...
Updates newhorse to the latest version or a specific version.
...
The newhorse CLI takes the following global flags.
...
Newhorse can be configured using environment variables.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Override basic auth username (default `newhorse`) |
