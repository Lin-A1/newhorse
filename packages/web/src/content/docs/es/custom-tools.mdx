...
description: Cree herramientas que LLM pueda llamar en newhorse.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de newhorse como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.newhorse/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/newhorse/tools/`.
...
```ts title=".newhorse/tools/database.ts" {1}
...
```ts title=".newhorse/tools/math.ts"
import { tool } from "@newhorse/plugin"
...
```ts title=".newhorse/tools/bash.ts"
import { tool } from "@newhorse/plugin"
...
```ts title=".newhorse/tools/project.ts" {8}
import { tool } from "@newhorse/plugin"
...
```python title=".newhorse/tools/add.py"
import sys
...
```ts title=".newhorse/tools/python-add.ts" {10}
import { tool } from "@newhorse/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".newhorse/tools/add.py")
