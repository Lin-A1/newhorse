---
title: CLI
description: Opciones y comandos de la CLI de Newhorse.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de Newhorse de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
newhorse
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con Newhorse mediante programación.
...
Inicie la interfaz de usuario del terminal Newhorse.
...
La CLI de Newhorse también tiene los siguientes comandos.

---

### agent

Administrar agentes para Newhorse.
...
Conecte una terminal a un servidor backend Newhorse que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
newhorse attach [url]
```

Esto permite usar TUI con un backend remoto Newhorse. Por ejemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `newhorse`) |
...
Newhorse funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `newhorse auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/newhorse/auth.json`.

```bash
newhorse auth login
```

Cuando se inicia Newhorse, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en Newhorse.
...
Ejecute newhorse en modo no interactivo pasando un mensaje directamente.
...
También puede conectarse a una instancia `newhorse serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Adjuntar a un servidor newhorse en ejecución (por ejemplo, http://localhost:4096)         |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Contraseña de autenticación básica (predeterminada: `OPENCODE_SERVER_PASSWORD`)           |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `newhorse`) |
...
Inicie un servidor Newhorse sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
newhorse serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad newhorse sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `newhorse`).
...
Enumere todas las sesiones Newhorse.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones Newhorse.
...
Importe datos de sesión desde un archivo JSON o una URL compartida Newhorse.

```bash
newhorse import <file>
```

Puede importar desde un archivo local o una URL compartida Newhorse.
...
Inicie un servidor Newhorse sin cabeza con una interfaz web.

```bash
newhorse web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a Newhorse a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `newhorse`).
...
Desinstale Newhorse y elimine todos los archivos relacionados.
...
Actualiza newhorse a la última versión o a una versión específica.
...
La CLI de Newhorse toma las siguientes banderas globales.
...
Newhorse se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `newhorse`) |
