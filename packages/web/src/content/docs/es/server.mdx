---
title: Servidor
description: Interactuar con el servidor newhorse a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `newhorse serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente newhorse puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `newhorse.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `newhorse`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `newhorse serve` como a `newhorse web`.
...
Cuando ejecuta `newhorse`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor newhorse para interactuar con newhorse mediante programación.
:::

Esta arquitectura permite que newhorse admita múltiples clientes y le permite interactuar con newhorse mediante programación.

Puede ejecutar `newhorse serve` para iniciar un servidor independiente. Si tienes el
newhorse TUI ejecutándose, `newhorse serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Newhorse [IDE](/docs/ide).
...
El servidor newhorse expone las siguientes API.
