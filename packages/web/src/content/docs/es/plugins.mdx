---
title: Complementos
description: Escriba sus propios complementos para extender Newhorse.
---

Los complementos le permiten extender Newhorse al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Newhorse.
...
- `.newhorse/plugins/` - Complementos a nivel de proyecto
- `~/.config/newhorse/plugins/` - Complementos globales
...
```json title="newhorse.json"
{
  ...
  "plugin": ["newhorse-helicone-session", "newhorse-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/newhorse/node_modules/`.
...
1. Configuración global (`~/.config/newhorse/newhorse.json`)
2. Configuración del proyecto (`newhorse.json`)
3. Directorio global de complementos (`~/.config/newhorse/plugins/`)
4. Directorio de complementos del proyecto (`.newhorse/plugins/`)
...
```json title=".newhorse/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Newhorse ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".newhorse/plugins/my-plugin.ts"
...
```js title=".newhorse/plugins/example.js"
...
- `client`: Un cliente SDK newhorse para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar newhorse.
...
```js title=".newhorse/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "newhorse"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Newhorse, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que newhorse lea archivos `.env`:

```javascript title=".newhorse/plugins/env-protection.js"
...
```javascript title=".newhorse/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a newhorse:

```ts title=".newhorse/plugins/custom-tools.ts"
import { type Plugin, tool } from "@newhorse/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que newhorse puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para newhorse junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://github.com/Lin-A1/newhorse/docs/sdk) para obtener más detalles.
...
```ts title=".newhorse/plugins/compaction.ts"
import type { Plugin } from "@newhorse/plugin"
...
```ts title=".newhorse/plugins/custom-compaction.ts"
import type { Plugin } from "@newhorse/plugin"
