...
Možete dodati vanjske alate u Newhorse koristeći _Model Context Protocol_, ili MCP. Newhorse podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Newhorse Config](https://github.com/Lin-A1/newhorse/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/newhorse`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Newhorse automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Newhorse će:
...
Ako server zahtijeva autentifikaciju, Newhorse će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `newhorse mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Newhorse sigurno čuva tokene u `~/.local/share/newhorse/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Newhorse, zajedno s ugrađenim alatima. Možete njima upravljati kroz Newhorse konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Newhorse sa vašim Sentry nalogom.
