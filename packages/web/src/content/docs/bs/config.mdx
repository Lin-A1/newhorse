---
title: Konfiguracija
description: Korištenje Newhorse JSON konfiguracije.
---

Možete konfigurirati Newhorse koristeći JSON konfiguracijski fajl.

---

## Format

Newhorse podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/newhorse`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/newhorse/newhorse.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OPENCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`newhorse.json` u projektu) - postavke specifične za projekat
5. **`.newhorse` direktoriji** - agenti, komande, dodaci
...
:::note
Direktoriji `.newhorse` i `~/.config/newhorse` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/newhorse` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Newhorse konfiguraciju u `~/.config/newhorse/newhorse.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/newhorse/tui.json`.
...
Dodajte `newhorse.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Newhorse pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OPENCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.newhorse` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.newhorse`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`newhorse.ai/config.json`**](https://github.com/Lin-A1/newhorse/config.json).

TUI konfiguracija koristi [**`newhorse.ai/tui.json`**](https://github.com/Lin-A1/newhorse/tui.json).
...
Stari `theme`, `keybinds`, i `tui` ključevi u `newhorse.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `newhorse serve` i `newhorse web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Newhorse server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `newhorse.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Newhorse konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Newhorse pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/newhorse/agents/` ili `.newhorse/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Newhorse će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`newhorse run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/newhorse/commands/` ili `.newhorse/commands/`. [Saznajte više ovdje](/docs/commands).
...
Newhorse će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Newhorse **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Newhorse sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.newhorse/plugins/` ili `~/.config/newhorse/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Newhorse da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
