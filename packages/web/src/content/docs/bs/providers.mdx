---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Newhorse.
---

import config from "../../../../config.mjs"
export const console = config.console

Newhorse koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Newhorse konfiguraciji.
...
u `~/.local/share/newhorse/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Newhorse
config.
...
Newhorse Zen je lista modela koje je obezbedio Newhorse tim koji su bili
testirano i potvrđeno da dobro radi sa Newhorse. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Newhorse Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite newhorse i idite na [newhorse.ai/auth](https://github.com/Lin-A1/newhorse/auth).
...
Radi kao i svaki drugi provajder u Newhorse i potpuno je opcionalan za korištenje.

---

## Newhorse Go

Newhorse Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Newhorse tim i koji su testirani i verificirani da dobro rade s Newhorse-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Newhorse Go` i idite na [newhorse.ai/auth](https://github.com/Lin-A1/newhorse/zen).
...
Da biste koristili Amazon Bedrock s Newhorse:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete newhorse:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `newhorse.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Newhorse.
:::
...
Možete konfigurirati newhorse za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi newhorse ispravno radio.
   :::
...
Također možete dodati modele kroz svoju newhorse konfiguraciju.
...
da pohrani token u newhorse auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Newhorse koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Newhorse
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`newhorse.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [newhorse-gitlab-auth](https://www.npmjs.com/package/newhorse-gitlab-auth).

##### Konfiguracija

Prilagodite putem `newhorse.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s newhorse:
...
Za korištenje Google Vertex AI s Newhorse:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće newhorse.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem newhorsea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [newhorse-helicone-session](https://github.com/H2Shami/newhorse-helicone-session) da automatski prijavite svaki Newhorse razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Newhorse razgovor naveden kao zasebna sesija.
...
Možete konfigurirati newhorse za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati newhorse za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati newhorse za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Newhorse. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/newhorse) za detalje.
:::
...
Da biste koristili Ollama Cloud s Newhorse:
...
3. Kopirajte API ključ za korištenje u Newhorse.
...
6. **Važno**: Prije upotrebe modela oblaka u Newhorse, morate lokalno povući informacije o modelu:
...
Newhorse Zen je lista testiranih i verifikovanih modela koju je obezbedio Newhorse tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Newhorse Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Newhorse Zen**.

```txt
   /connect
```

3. Unesite svoj Newhorse API ključ.
...
Također možete dodati dodatne modele putem vaše newhorse konfiguracije.
...
5. Također ih možete prilagoditi putem vaše newhorse konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Newhorseom:
...
Također možete prilagoditi modele kroz svoju newhorse konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s newhorse-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `newhorse.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Newhorse da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `newhorse auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Newhorse konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj newhorse konfiguraciji.
