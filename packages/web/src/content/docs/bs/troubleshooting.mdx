...
Da biste otklonili probleme s Newhorse, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\newhorse\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `newhorse --log-level DEBUG`.

---

## Pohrana

Newhorse pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:
...
Newhorse Desktop pokreće lokalni Newhorse server (`newhorse-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Newhorse` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/newhorse/newhorse.jsonc` (ili `~/.config/newhorse/newhorse.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/newhorse/newhorse.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\newhorse\newhorse.jsonc`
...
Newhorse također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Newhorse mogao ponovo izgraditi.

1. Potpuno zatvorite Newhorse Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/newhorse`
- **Linux**: obrišite `~/.cache/newhorse` (ili pokrenite `rm -rf ~/.cache/newhorse`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\newhorse`

3. Ponovo pokrenite Newhorse Desktop.

---

### Rješavanje problema sa vezom na serveru

Newhorse Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `newhorse.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, Newhorse Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Newhorse karakteristikama.

---

### Obavještenja se ne prikazuju

Newhorse Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Newhorse u postavkama vašeg OS-a, i
...
1. Zatvorite Newhorse Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Newhorse Desktop aplikacije):

- `newhorse.settings.dat` (URL zadanog servera za desktop)
- `newhorse.global.dat` i `newhorse.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `newhorse models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\newhorse`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Newhorse dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\newhorse`

2. Ponovo pokrenite Newhorse da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Newhorse da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Newhorse će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
