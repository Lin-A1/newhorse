---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Newhorse.
---

Dodaci vam omogućavaju da proširite Newhorse spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Newhorse.
...
- `.newhorse/plugins/` - Dodaci na nivou projekta
- `~/.config/newhorse/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/newhorse/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/newhorse/newhorse.json`)
2. Konfiguracija projekta (`newhorse.json`)
3. Globalni direktorij dodataka (`~/.config/newhorse/plugins/`)
4. Direktorij dodataka projekta (`.newhorse/plugins/`)
...
Newhorse pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Newhorse SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Newhorse.
...
Spriječite newhorse da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u newhorse:
...
Pomoćnik `tool` kreira prilagođeni alat koji newhorse može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za newhorse zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://github.com/Lin-A1/newhorse/docs/sdk) za detalje.
