...
Postoje dvije vrste agenata u Newhorse: primarni agenti i podagenti.
...
Newhorse dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Newhorse dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Newhorse dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u Newhorse-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `newhorse.json`:
...
- Globalno: `~/.config/newhorse/agents/`
- Po projektu: `.newhorse/agents/`

```markdown title="~/.config/newhorse/agents/review.md"
---
...
Ako temperatura nije navedena, Newhorse koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Newhorse konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Newhorse konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Newhorse Zen](/docs/zen), koristili biste `newhorse/gpt-5.1-codex` za GPT 5.1 Codex.
...
```json title="newhorse.json" {3-6,9-12}
{
  "$schema": "https://github.com/Lin-A1/newhorse/config.json",
  "tools": {
    "write": true,
...
```json title="newhorse.json" {3-5,8-10}
{
  "$schema": "https://github.com/Lin-A1/newhorse/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Pokrenite `newhorse models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/Lin-A1/newhorse).
:::
...
```markdown title="~/.config/newhorse/agents/docs-writer.md"
---
...
```markdown title="~/.config/newhorse/agents/security-auditor.md"
---
