...
U Newhorse mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Newhorse koristi nasu `newhorse` temu.
...
Newhorse dolazi sa vise ugradenih tema.
...
- Zele da Newhorse odgovara izgledu njihovog terminala
...
Newhorse podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/newhorse/themes/*.json` ili `$XDG_CONFIG_HOME/newhorse/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.newhorse/themes/*.json`
4. **Trenutni radni direktorij** - `./.newhorse/themes/*.json`
