---
title: Server
description: Komunicirajte s newhorse serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `newhorse serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji newhorse klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `newhorse.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `newhorse`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `newhorse serve` i za `newhorse web`.
...
Kada pokrenete `newhorse`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite newhorse server za programsku interakciju sa newhorse-om.
:::

Ova arhitektura omogucava newhorse podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `newhorse serve` da startate standalone server. Ako je newhorse TUI vec pokrenut, `newhorse serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Newhorse [IDE](/docs/ide) pluginovi.
...
newhorse server izlaže sljedece API-je.
