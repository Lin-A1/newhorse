---
title: CLI
description: Newhorse CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Newhorse CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
newhorse
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa Newhorse.
...
Pokrenite Newhorse terminalski korisnički interfejs.
...
Newhorse CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte Newhorse agentima.
...
Priključite terminal na već pokrenut Newhorse backend server pokrenut putem `serve` ili `web` naredbi.

```bash
newhorse attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim Newhorse backend-om. Na primjer:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `newhorse`) |
...
Newhorse pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `newhorse auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/newhorse/auth.json`.

```bash
newhorse auth login
```

Kada se Newhorse pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u Newhorse.
...
Pokrenite Newhorse u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `newhorse serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| <nobr><code>{"--attach"}</code></nobr>   |        | Priključite na pokrenuti Newhorse server (npr. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `newhorse`) |
...
Pokrenite Newhorse headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
newhorse serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti Newhorse-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `newhorse`).
...
Prikaži statistiku upotrebe tokena i troškova za vaše Newhorse sesije.
...
Uvezite podatke sesije iz JSON datoteke ili Newhorse dijeljenog URL-a.

```bash
newhorse import <file>
```

Možete uvesti iz lokalne datoteke ili Newhorse dijeljenog URL-a.
...
Pokrenite Newhorse headless server sa web interfejsom.

```bash
newhorse web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup Newhorse-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `newhorse`).
...
Deinstalirajte Newhorse i uklonite sve povezane datoteke.
...
Ažurira Newhorse na najnoviju verziju ili određenu verziju.
...
Newhorse CLI prihvata sljedeće globalne zastavice.
...
Newhorse se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `newhorse`) |
