---
title: Server
description: Nutze den newhorse-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `newhorse serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein newhorse-Client nutzen kann.
...
Der Benutzername ist standardmaessig `newhorse`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `newhorse serve` und `newhorse web`.
...
Wenn du `newhorse` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den newhorse-Server, um newhorse programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `newhorse serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `newhorse serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Newhorse-[IDE](/docs/ide)-Plugins.
...
Der newhorse-Server stellt folgende APIs bereit.
