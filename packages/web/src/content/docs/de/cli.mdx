---
title: CLI
description: Newhorse CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der Newhorse CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
newhorse
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit Newhorse interagieren.

```bash
newhorse run "Explain how closures work in JavaScript"
```
...
Starten Sie die Terminalbenutzeroberfläche Newhorse.
...
Der Newhorse CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für Newhorse verwalten.

```bash
newhorse agent [command]
```
...
Schließen Sie ein Terminal an einen bereits laufenden Newhorse-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
newhorse attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend Newhorse. Zum Beispiel:

```bash
# Start the backend server for web/mobile access
newhorse web --port 4096 --hostname 0.0.0.0

# In another terminal, attach the TUI to the running backend
newhorse attach http://10.20.30.40:4096
```
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `newhorse`) |
...
Newhorse wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `newhorse auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/newhorse/auth.json` gespeichert.

```bash
newhorse auth login
```

Wenn Newhorse startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
```bash
newhorse mcp auth list
```

Oder nutzen Sie die Kurzversion.

```bash
newhorse mcp auth ls
```
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in Newhorse sehen möchten.

```bash
newhorse models --refresh
```
...
Führen Sie Newhorse im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.

```bash
newhorse run [message..]
```
...
```bash "newhorse run"
newhorse run Explain the use of context in Go
```

Sie können auch eine Verbindung zu einer laufenden `newhorse serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
newhorse run --attach http://localhost:4096 "Explain async/await in JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |      | An einen laufenden Newhorse-Server anschließen (z.B. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Basic-Auth-Passwort (standardmäßig `OPENCODE_SERVER_PASSWORD`)                                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `newhorse`)                  |
...
Starten Sie einen Headless-Newhorse-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
newhorse serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf Newhorse-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `newhorse`).
...
Newhorse-Sitzungen verwalten.

```bash
newhorse session [command]
```
...
Listen Sie alle Newhorse-Sitzungen auf.

```bash
newhorse session list
```
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre Newhorse-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer Newhorse-Freigabe-URL.

```bash
newhorse import <file>
```

Sie können aus einer lokalen Datei oder einer Newhorse-Freigabe-URL importieren.

```bash
newhorse import session.json
newhorse import https://opncd.ai/s/abc123
```
...
Starten Sie einen Headless-Newhorse-Server mit einer Weboberfläche.

```bash
newhorse web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf Newhorse zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `newhorse`).
...
Deinstallieren Sie Newhorse und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert Newhorse auf die neueste Version oder eine bestimmte Version.

```bash
newhorse upgrade [target]
```
...
Der Newhorse CLI akzeptiert die folgenden globalen Flags.
...
Newhorse kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `newhorse`) |
