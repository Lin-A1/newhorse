---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Newhorse zu erweitern.
---

Mit Plugins können Sie Newhorse erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Newhorse zu ändern.
...
- `.newhorse/plugins/` – Plugins auf Projektebene
- `~/.config/newhorse/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/newhorse/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/newhorse/newhorse.json`)
2. Projektkonfiguration (`newhorse.json`)
3. Globales Plugin-Verzeichnis (`~/.config/newhorse/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.newhorse/plugins/`)
...
Newhorse führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Newhorse-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Newhorse erweitern können.
...
:::note
Wenn Sie die Newhorse-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Newhorse `.env`-Dateien liest:
...
Plugins können Newhorse auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Newhorse aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Newhorse zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://github.com/Lin-A1/newhorse/docs/sdk).
