---
title: Regeln
description: Lege eigene Anweisungen fuer newhorse fest.
---

Du kannst newhorse ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in newhorse den Befehl `/init` aus.
...
So kann newhorse sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht newhorse sie zu erweitern.
...
newhorse kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/newhorse/AGENTS.md` speichern.
Diese gelten in allen newhorse-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Newhorse dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/newhorse/AGENTS.md` existiert)
...
Beim Start sucht newhorse in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/newhorse/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `newhorse.json` oder in `~/.config/newhorse/newhorse.json` angeben.
...
newhorse parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit newhorse.json

Empfohlen ist das `instructions`-Feld in der `newhorse.json`:
...
Du kannst newhorse explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `newhorse.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
