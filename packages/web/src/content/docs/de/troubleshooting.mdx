...
Wenn Newhorse Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `newhorse --log-level DEBUG`.

---

## Speicher

newhorse speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
Newhorse Desktop startet im Hintergrund einen lokalen Newhorse-Server (`newhorse-cli`-Sidecar).
...
- Nur macOS: `Newhorse`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/newhorse/newhorse.jsonc` (oder `~/.config/newhorse/newhorse.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/newhorse/newhorse.jsonc`
...
Newhorse kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Newhorse ihn neu aufbauen kann.

1. Beende Newhorse Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/newhorse`
- **Linux**: loesche `~/.cache/newhorse` (oder fuehre aus: `rm -rf ~/.cache/newhorse`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\newhorse`

3. Starte Newhorse Desktop neu.

---

### Server-Verbindungsprobleme beheben

Newhorse Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `newhorse.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt Newhorse Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Newhorse-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Newhorse Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Newhorse in den OS-Einstellungen aktiviert sind, und
...
1. Beende Newhorse Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Newhorse Desktop):

- `newhorse.settings.dat` (desktop default server URL)
- `newhorse.global.dat` und `newhorse.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Newhorse hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `newhorse models` aus.
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. newhorse installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
2. Starte newhorse neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt newhorse, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
newhorse erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
