...
You can add external tools to Newhorse using the _Model Context Protocol_, or MCP. Newhorse supports both local and remote servers.
...
You can define MCP servers in your [Newhorse Config](https://github.com/Lin-A1/newhorse/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/newhorse` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Newhorse automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Newhorse will:
...
If the server requires authentication, Newhorse will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `newhorse mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Newhorse will store the tokens securely in `~/.local/share/newhorse/mcp-auth.json`.
...
Your MCPs are available as tools in Newhorse, alongside built-in tools. So you can manage them through the Newhorse config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Newhorse to your Sentry account.
