---
title: Server
description: Interact with newhorse server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `newhorse serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an newhorse client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `newhorse.local` |
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `newhorse`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `newhorse serve` and `newhorse web`.
...
When you run `newhorse` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the newhorse server to interact with newhorse programmatically.
:::

This architecture lets newhorse support multiple clients and allows you to interact with newhorse programmatically.

You can run `newhorse serve` to start a standalone server. If you have the
newhorse TUI running, `newhorse serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Newhorse [IDE](/docs/ide) plugins.
...
The newhorse server exposes the following APIs.
