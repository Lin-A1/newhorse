---
title: Tjener
description: Samhandle med Newhorse-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `newhorse serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Newhorse-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `newhorse.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `newhorse` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `newhorse serve` og `newhorse web`.
...
Når du kjører `newhorse` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Newhorse-serveren til å samhandle med Newhorse programmatisk.
:::

Denne arkitekturen lar Newhorse støtte flere klienter og lar deg samhandle med Newhorse programmatisk.

Du kan kjøre `newhorse serve` for å starte en frittstående server. Hvis du har
Newhorse TUI kjører, vil `newhorse serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Newhorse [IDE](/docs/ide) plugins.
...
newhorse-serveren viser følgende APIer.
