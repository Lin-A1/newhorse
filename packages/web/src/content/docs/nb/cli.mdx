---
title: CLI
description: Newhorse CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Newhorse CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
newhorse
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med Newhorse programmatisk.
...
Start Newhorse-terminalbrukergrensesnittet.
...
Administrer agenter for Newhorse.
...
Koble en terminal til en allerede kjørende Newhorse-backend-server startet via `serve` eller `web` kommandoer.

```bash
newhorse attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern Newhorse-backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `newhorse`) |
...
Newhorse drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `newhorse auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/newhorse/auth.json`.

```bash
newhorse auth login
```

Når Newhorse starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i Newhorse.
...
Kjør Newhorse i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `newhorse serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Koble til en kjørende Newhorse-server (f.eks. http://localhost:4096)                               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Passord for grunnleggende autentisering (standard: `OPENCODE_SERVER_PASSWORD`)                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `newhorse`) |
...
Start en headless Newhorse-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
newhorse serve
```

Dette starter en HTTP-server som gir API tilgang til Newhorse-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `newhorse`).
...
Vis tokenbruk og kostnadsstatistikk for Newhorse-øktene dine.
...
Importer øktdata fra en JSON-fil eller en Newhorse share-URL.
...
Start en headless Newhorse-server med et webgrensesnitt.

```bash
newhorse web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til Newhorse via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `newhorse`).
...
Avinstaller Newhorse og fjern alle relaterte filer.
...
Oppdaterer Newhorse til siste versjon eller en spesifikk versjon.
...
Newhorse CLI bruker følgende globale flagg.
...
Newhorse kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `newhorse`) |
