...
Du kan legge til eksterne verktøy i Newhorse ved å bruke _Model Context Protocol_ eller MCP. Newhorse støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Newhorse Config](https://github.com/Lin-A1/newhorse/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="newhorse.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/newhorse`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Newhorse håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Newhorse:
...
Hvis serveren krever autentisering, vil Newhorse be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `newhorse mcp auth <server-name>`.
...
```bash
newhorse mcp auth my-oauth-server
```
...
```bash
newhorse mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Newhorse lagre tokens sikkert i `~/.local/share/newhorse/mcp-auth.json`.
...
newhorse mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Newhorse, sammen med innebygde verktøy. Så du kan administrere dem gjennom Newhorse-konfigurasjonen som alle andre verktøy.
...
```json title="newhorse.json" {11, 14-18}
{
  "$schema": "https://github.com/Lin-A1/newhorse/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
newhorse mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Newhorse til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Newhorse-samtale oppført som en separat økt.
