---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Newhorse.
---

import config from "../../../../config.mjs"
export const console = config.console

Newhorse bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Newhorse-konfigurasjonen.
...
i `~/.local/share/newhorse/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Newhorse
konfig.
...
Newhorse Zen er en liste over modeller levert av Newhorse-teamet som har vært
testet og verifisert for å fungere godt med Newhorse. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Newhorse Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Newhorse Zen` og gå til [newhorse.ai/auth](https://github.com/Lin-A1/newhorse/zen).
...
Det fungerer som alle andre leverandører i Newhorse og er helt valgfritt å bruke.

---

## Newhorse Go

Newhorse Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Newhorse-teamet som har vært
testet og verifisert for å fungere godt med Newhorse.

1. Kjør kommandoen `/connect` i TUI, velg `Newhorse Go`, og gå til [newhorse.ai/auth](https://github.com/Lin-A1/newhorse/zen).
...
Slik bruker du Amazon Bedrock med Newhorse:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Newhorse:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `newhorse.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i newhorse støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere newhorse til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Newhorse skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Newhorse-konfigurasjonen.
...
for å lagre token i Newhorse auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Newhorse bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Newhorse
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`newhorse.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [newhorse-gitlab-auth](https://www.npmjs.com/package/newhorse-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `newhorse.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Newhorse:
...
Slik bruker du Google Vertex AI med Newhorse:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Newhorse.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Newhorse, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [newhorse-helicone-session](https://github.com/H2Shami/newhorse-helicone-session) plugin for automatisk å logge hver Newhorse-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Newhorse-samtale oppført som en separat økt.
...
Du kan konfigurere Newhorse for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Newhorse for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Newhorse for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Newhorse. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/newhorse) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Newhorse:
...
3. Kopier API-nøkkelen for bruk i Newhorse.
...
6. **Viktig**: Før du bruker skymodeller i Newhorse, må du hente modellinformasjonen lokalt:
...
Newhorse Zen er en liste over testede og verifiserte modeller levert av Newhorse-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Newhorse Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Newhorse Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Newhorse API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Newhorse-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Newhorse-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Newhorse:
...
Du kan også tilpasse modeller gjennom Newhorse-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Newhorse. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `newhorse.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Newhorse forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `newhorse auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Newhorse-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Newhorse-konfigurasjonen.
