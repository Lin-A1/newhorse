...
description: Skriv dine egne programtillegg for å utvide Newhorse.
---

Plugins lar deg utvide Newhorse ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Newhorse.
...
- `.newhorse/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/newhorse/node_modules/`.
...
1. Global konfigurasjon (`~/.config/newhorse/newhorse.json`)
2. Prosjektkonfigurasjon (`newhorse.json`)
3. Global plugin-katalog (`~/.config/newhorse/plugins/`)
4. Prosjektpluginkatalog (`.newhorse/plugins/`)
...
Newhorse kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Newhorse SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Newhorse.
...
:::note
Hvis du bruker Newhorse-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Newhorse fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Newhorse:
...
`tool`-hjelperen lager et tilpasset verktøy som Newhorse kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Newhorse sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://github.com/Lin-A1/newhorse/docs/sdk) for detaljer.
