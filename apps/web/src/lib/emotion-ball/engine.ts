 ...
 * ============================================================ */
import { EB_RINGS as RD } from "./rings";
...
import { createBall } from "./ball";
  ...
  Engine.prototype = {
    ...
    registerEmotion: function (raw) { return CONFIG.register(raw); },
...
export function createEmotionBall(target: HTMLElement, opts?: Record<string, unknown>): Engine {
  return new Engine(target, opts);
}

/* 载入种子配置（emotions 数据在模块加载时注册） */
if (Array.isArray(EMOTION_SEED)) {
  EMOTION_SEED.forEach(function (raw) {
    var r = register(raw);
